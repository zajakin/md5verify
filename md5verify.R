...
system(paste0("cd /wd/ && (find /wd/ -not -path '*/#recycle/*' -name md5sum.txt; find /wd/ -not -path '*/#recycle/*' -name *.md5; find /wd/ -not -path '*/#recycle/*' -name *_FileInfo.txt) > /wd/md5files.txt 2> /wd/md5verify.log"),intern=TRUE)
...
system(paste0("cd /wd/ && find /wd/ -not -path '*/#recycle/*' -name *_md5_check_report_NAS.txt -exec grep --with-filename -v ' OK$' {} \\; > /wd/md5errors.txt 2>&1 && sed -i 's!^/wd/!!g' /wd/md5errors.txt"),intern=TRUE)
